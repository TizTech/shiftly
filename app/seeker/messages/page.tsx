      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-3">
        ...
        <div className="space-y-2">
          {conversations.map((conversation) => (
            <a
              key={conversation.id}
              href={`/seeker/messages?conversationId=${conversation.id}`}
              className={`block rounded-xl px-3 py-2 ${activeConversation?.id === conversation.id ? "bg-slate-900 text-white" : "hover:bg-slate-100"}`}
            >
              <p className="text-sm font-semibold">{conversation.employer.employerProfile?.company?.name || conversation.employer.fullName}</p>
